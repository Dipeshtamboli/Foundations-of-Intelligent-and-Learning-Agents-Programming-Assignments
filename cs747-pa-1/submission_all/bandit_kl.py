...
for t in range(1, horizon+1):
    # if t%10==0:
    #     successes -= np.max(successes)
    #     failures -= np.max(failures)
    for real_arm in range(len(arms_prob)):
        # posterior[real_arm] = (sorted_p**successes[real_arm])*(sorted_1_p**failures[real_arm])
        posterior[real_arm] = (successes[real_arm]*np.log(sorted_p) + failures[real_arm]*np.log(sorted_1_p))
    posterior -= (np.min(posterior, axis=1).reshape(len(arms_prob),1) )
...
def thompson_sampling_with_hint(instance, algorithm,arm_prob,randomSeed,epsilon,horizon,sorted_true_means):
    ...
    for t in range(1, horizon+1):
        ...
        # print(posterior)
        # print(arm_id, np.max(posterior[:,-1]),np.max(posterior) , posterior[arm_id, -1])
        # print(posterior, arm_id, np.max(posterior), posterior[arm_id, -1])
        ...
        if t in [100, 400, 1600, 6400, 25600, 102400]:
            print(f'{instance}, {algorithm}, {randomSeed}, {epsilon}, {t}, {regret_step[t-1]}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--instance', default="../instances/i-2.txt", help='instance in, where in is a path to the instance file.')
